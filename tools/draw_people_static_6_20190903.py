...
def loadradar(filename):
  ...
  with open(filename) as myfile:
    for line in myfile:
            ...
            dist_y.append(float(dist_y_item))
    ...
    return dist_x, dist_y, distance, angle, power

if __name__ == "__main__":
    if len(sys.argv) == 2:
        dist_x, dist_y, distance, angle, power = loadradar(sys.argv[1])
        ...
        if False:
            ...
            plt.text(25, 4.98, "Ground truth: suppose 5.0m\n    min: " + str(np.min(dist_x)) + ", mean: " +
                 "{0:.4f}".format(np.mean(dist_x)) + ", max: " + str(np.max(dist_x)) + ", std dev: " + "{0:.4f}".format(np.std(dist_x)) +
                 "\n    error: " + "{:.4%}".format((np.mean(dist_x) - 5.0) / 5.0), fontdict=font)
        plt.grid(True)
        fig = plt.figure("dist_y")
        plt.plot(dist_y)
        plt.ylabel('dist_y')
        # plt.yticks(np.arange(int(np.min(distance)), int(np.max(distance)) + 1, 1))
        font = {'family': 'serif',
        ...
        if True:
            plt.text(30, -0.2, "min: " + str(np.min(dist_y)) + ", mean: " +
                 "{0:.4f}".format(np.mean(dist_y)) + ", max: " + str(np.max(dist_y)), fontdict=font)
        else:
            plt.text(9500, 5.10, "Ground truth: suppose 5.0m\n    min: " + str(np.min(dist_x)) + ", mean: " +
                 "{0:.4f}".format(np.mean(dist_x)) + ", max: " + str(np.max(dist_x)) + ", std dev: " + "{0:.4f}".format(np.std(dist_x)) +
                 "\n    error: " + "{:.4%}".format((np.mean(dist_x) - 5.0) / 5.0), fontdict=font)
        plt.grid(True)
        ...
        if True:
            if True:
                plt.text(30, 2.5, "Ground truth: 0.0deg\n    min: " + str(np.min(angle)) + ", mean: "
        ...
        plt.text(40, -36.2, "min: " + str(np.min(power)) + ", mean: " + "{0:.4f}".format(np.mean(power)) + ", max: " + str(np.max(power)),
            fontdict=font)
        plt.grid(True)
        plt.show()
    else:
